...
def driver_factory(config):
    ...
    def inner(config=config, return_value=None, use_numeric_key=False):
        ...
        driver.send_command_raw = MagicMock(return_value=return_value)
...
def test_simple(driver):
    assert len(driver.get_data('commands')['commands']) == 7


def test_execute_command(config, driver):
    driver.execute_command('command1')
    driver.send_command_raw.assert_called_with('command1', config['commands']['command1'], None)


def test_execute_command_list(config, driver):
    driver.execute_command('command7')
    sub_command = copy.deepcopy(config['commands']['command7']['commands'][0])
    sub_command['has_more'] = True
    driver.send_command_raw.assert_has_calls(
...
def test_execute_command_with_arg_no_translate(config, driver):
    driver.execute_command('command1', 'arg')
    driver.send_command_raw.assert_called_with('command1', config['commands']['command1'], 'arg')


def test_execute_command_with_arg_translate_failed(config, driver):
    driver.execute_command('command2', 'arg')
    driver.send_command_raw.assert_called_with('command2', config['commands']['command2'], 'arg')


def test_execute_command_translate(config, driver):
    driver.execute_command('command2', 'key1')
    driver.send_command_raw.assert_called_with('command2', config['commands']['command2'], 'value1')


def test_execute_command_translate_numberic(config, driver_factory):
    driver = driver_factory(config, None, True)
    driver.execute_command('command2', '0')
    driver.send_command_raw.assert_called_with('command2', config['commands']['command2'], 'value1')


def test_get_data_no_translate(config, driver):
    response = driver.get_data('command3')
    assert response['output'] == 'some_value'
    assert 'result' not in response
    driver.send_command_raw.assert_called_with('command3', config['commands']['command3'])


def test_get_data_translate_failed(config, driver):
    response = driver.get_data('command4')
    assert response['output'] == 'some_value'
    assert 'result' not in response
    driver.send_command_raw.assert_called_with('command4', config['commands']['command4'])


def test_get_data_translate_default(config, driver):
    response = driver.get_data('command6')
    assert response['output'] == 'some_value'
    assert response['result'] == 'key6'
    driver.send_command_raw.assert_called_with('command6', config['commands']['command6'])
...
def test_get_data_translate_one_way(config, driver_factory):
    ...
    response = driver.get_data('command5')
    assert response['output'] == 'value3'
    assert response['result'] == 'key3'
    driver.send_command_raw.assert_called_with('command5', config['commands']['command5'])
...
def test_execute_command_bool_param(num_config, driver_factory):
    ...
    driver.execute_command('boolCommand', 'on')
    driver.send_command_raw.assert_called_with('boolCommand', num_config['commands']['boolCommand'], True)


def test_execute_command_bool_param_translate(num_config, driver_factory):
    driver = driver_factory(num_config)
    driver.execute_command('boolCommandTranslate', '1')
    driver.send_command_raw.assert_called_with('boolCommandTranslate', num_config['commands']['boolCommandTranslate'],
                                               True)
...
def test_execute_command_int_param(num_config, driver_factory):
    ...
    driver.execute_command('intCommand', '123')
    driver.send_command_raw.assert_called_with('intCommand', num_config['commands']['intCommand'], 123)


def test_execute_command_hex_param(num_config, driver_factory):
    driver = driver_factory(num_config, '0')

    driver.execute_command('hexCommand', '11')
    driver.send_command_raw.assert_called_with('hexCommand', num_config['commands']['hexCommand'], 'B')


def test_execute_command_float_param(num_config, driver_factory):
    driver = driver_factory(num_config, '0')

    driver.execute_command('floatCommand', '1.1')
    driver.send_command_raw.assert_called_with('floatCommand', num_config['commands']['floatCommand'], '1.1')


def test_execute_command_float_param_no_fract(num_config, driver_factory):
    driver = driver_factory(num_config, '0')

    driver.execute_command('floatCommand', '1')
    driver.send_command_raw.assert_called_with('floatCommand', num_config['commands']['floatCommand'], '1')


def test_get_data_int_param(num_config, driver_factory):
    driver = driver_factory(num_config, '123')

    response = driver.get_data('intResult')
...
def test_get_data_hex_param(num_config, driver_factory):
    ...
    response = driver.get_data('hexResult')
...
def test_get_data_float_param(num_config, driver_factory):
    ...
    response = driver.get_data('floatResult')
