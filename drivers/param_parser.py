...
class ParamParser(object):
    ...
    REPLACE_CHARS = [{'match': ' ', 'replace': '_'}, {'match': '()', 'replace': ''}]

    def __init__(self, config, use_numeric_key=False):
        self.value_sets = {}
        for value_set_key, value_set_data in config.get('values', {}).items():
            self.create_value_sets(self.value_sets, value_set_key, value_set_data, use_numeric_key)

        for command_name, command_data in config.get('commands', {}).items():
            self.process_command_values(command_name, command_data, use_numeric_key)
    ...
    def process_command_values(self, command_name, command_data, use_numeric_key):
        if command_data is not None and 'values' in command_data:
            self.create_value_sets(self.value_sets, command_name, command_data['values'], use_numeric_key)
            command_data['value_set'] = command_name

    def create_value_sets(self, value_sets, value_set_key, value_set_data, use_numeric_key):
